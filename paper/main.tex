...
\mathcal{L}_D &= \mathbb{E}[\text{ReLU}(1 - D(\mathbf{x}, \mathbf{c}))] \notag \\
              &\quad + \mathbb{E}[\text{ReLU}(1 + D(G(\mathbf{z}, \mathbf{c}), \mathbf{c}))] \\
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/fig9_iki_cdf.pdf}
...
\begin{tabular}{lrrrr}
\toprule
Configuration & KS\,($\downarrow$) & $p$-value & $\Delta$ vs Full & Note \\
\midrule
...
\midrule
\multicolumn{5}{l}{\textit{Error Engine (event-level, not KS)}} \\
Full pipeline       & — & — & — & 2.00\% error rate;\ 2.00 corrections/100\,keys \\
No error/correction & — & — & — & 0.00\% error rate;\ 0.00 corrections/100\,keys \\
\bottomrule
\end{tabular}
\label{tab:ablation}
\end{table*}

\textbf{Finding.} The GAN's marginal distribution matching is robust to context
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/fig10_state_boxplot.pdf}
...
(\texttt{create\_graph=True}) that are not supported on resource-constrained
accelerators including Apple Silicon MPS and many edge-inference devices.
...
relative influence.
Increasing \texttt{noise\_dim} to 64 while reducing the hidden projection would restore
